    ...
    <div className="flex flex-col gap-6">
      <div className="card p-5">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-ink-900">Compare with another paper</h2>
      ...
      <div className="card p-5">
        <h2 className="mb-3 font-semibold text-ink-900">Conversation history</h2>
        {isLoading ? (
          <p className="text-sm text-ink-600">Loading conversations...</p>
        ) : !data || data.length === 0 ? (
          ...
          <ul className="flex flex-col divide-y divide-slate-100">
            {data.map((c) => (
              <li key={c.id} className="py-3">
                <p className="text-sm font-medium text-ink-900">{c.title || "Untitled conversation"}</p>
                <p className="text-xs text-ink-600">{new Date(c.created_at).toLocaleString()}</p>
              </li>
            ))}
          </ul>
        )}
