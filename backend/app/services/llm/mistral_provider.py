...
class MistralProvider(LLMProvider):
    ...
    def __init__(self, api_key: str, chat_model: str, embedding_model: str, timeout: float = 60.0):
        if not api_key:
            raise ValueError("MISTRAL_API_KEY is required to use the Mistral provider")
        self.api_key = api_key
        self.chat_model = chat_model
        self.embedding_model = embedding_model
        self._client = httpx.Client(base_url=MISTRAL_BASE_URL, timeout=timeout)
    ...
    def generate_structured(
        ...
        # Mistral's JSON mode guarantees valid JSON but not schema conformance,
        # so the schema is spelled out in the prompt as well.
        schema_instruction = (
