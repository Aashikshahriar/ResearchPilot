...
function AddMetricForm({ experimentId }: { experimentId: string }) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-wrap items-end gap-3">
      <div>
        <label className="mb-1 block text-xs font-medium text-ink-800">Metric name</label>
        <input className="input w-40" placeholder="accuracy" value={name} onChange={(e) => setName(e.target.value)} />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-ink-800">Value</label>
        <input className="input w-28" type="number" step="any" value={value} onChange={(e) => setValue(e.target.value)} />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-ink-800">Step</label>
...
function ExperimentDetailContent({ id }: { id: string }) {
  ...
  if (!experiment) return <div className="text-sm text-ink-600">Loading experiment...</div>;

  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-ink-900">{experiment.name}</h1>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="card p-4">
          <p className="text-xs text-ink-600">Learning rate</p>
          <p className="text-lg font-semibold text-ink-900">{experiment.learning_rate ?? "—"}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-ink-600">Batch size</p>
          <p className="text-lg font-semibold text-ink-900">{experiment.batch_size ?? "—"}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-ink-600">Epochs</p>
          <p className="text-lg font-semibold text-ink-900">{experiment.epochs ?? "—"}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-ink-600">Logged metrics</p>
          <p className="text-lg font-semibold text-ink-900">{metrics?.length ?? 0}</p>
        </div>
      </div>

      {experiment.notes && (
        <div className="card p-5">
          <h2 className="font-semibold text-ink-900">Notes</h2>
          <p className="mt-2 whitespace-pre-line text-sm text-ink-700">{experiment.notes}</p>
        </div>
      )}

      <div className="card p-5">
        <h2 className="mb-4 font-semibold text-ink-900">Metrics over time</h2>
        {isLoading ? (
          <p className="text-sm text-ink-600">Loading metrics...</p>
