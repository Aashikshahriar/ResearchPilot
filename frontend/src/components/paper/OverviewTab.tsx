  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="card p-5 lg:col-span-2">
        <h2 className="font-semibold text-ink-900">Abstract</h2>
      ...
      <div className="card p-5">
        <h2 className="font-semibold text-ink-900">Metadata</h2>
        <dl className="mt-3 flex flex-col gap-3 text-sm">
          <div>
            <dt className="text-ink-600">Authors</dt>
            <dd className="text-ink-900">{paper.authors?.length ? paper.authors.join(", ") : "Unknown"}</dd>
          </div>
          <div>
            <dt className="text-ink-600">Pages</dt>
            <dd className="text-ink-900">{paper.page_count ?? "—"}</dd>
          </div>
          <div>
            <dt className="text-ink-600">File size</dt>
            <dd className="text-ink-900">{(paper.file_size_bytes / 1024 / 1024).toFixed(2)} MB</dd>
          </div>
          <div>
            <dt className="text-ink-600">Uploaded</dt>
            <dd className="text-ink-900">{new Date(paper.created_at).toLocaleString()}</dd>
          </div>
        </dl>
      </div>

      <div className="card p-5 lg:col-span-3">
        <h2 className="mb-3 font-semibold text-ink-900">Sections ({paper.sections.length})</h2>
        {paper.sections.length === 0 ? (
          <p className="text-sm text-ink-600">Sections will appear here once processing completes.</p>
        ) : (
            ...
            {paper.sections.map((s) => (
              <details key={s.id} className="group py-3">
                <summary className="cursor-pointer list-none text-sm font-medium text-ink-900">
                  ...
                  {s.page_start && <span className="ml-2 text-xs font-normal text-ink-600">p.{s.page_start}-{s.page_end}</span>}
                </summary>
                <p className="mt-2 whitespace-pre-line text-sm text-ink-700">{s.content.slice(0, 1200)}</p>
              </details>
            ))}
          </div>
        )}
